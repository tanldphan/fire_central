...
int lora_init(void)
{
    ...
    spi_bus_config_t bus = { .miso_io_num = LORA_MISO,
                             .mosi_io_num = LORA_MOSI,
                             .sclk_io_num = LORA_SCLK,
                             .quadwp_io_num = -1,
                             .quadhd_io_num = -1,
                             .max_transfer_sz = 0 };
    ret = spi_bus_initialize (LORA_SPI, &bus, SPI_DMA_CH_AUTO);
    assert (ret == ESP_OK);
...
#if BUFFER_IO
    lora_write_reg_buffer (LORA_REG_FIFO, buf, size);
#else
    for (int i = 0; i < size; i++)
        lora_write_reg (LORA_REG_FIFO, *buf++);
#endif

    lora_write_reg (LORA_REG_PAYLOAD_LENGTH, size);

    // Start transmission and wait for conclusion.
    lora_write_reg (LORA_REG_OP_MODE, LORA_MODE_LONG_RANGE | LORA_MODE_TX);
#if 0
...
int lora_receive_packet(uint8_t* buf, int size)
{
    ...
    if ((irq & LORA_IRQ_RX_DONE_MASK) == 0)
        return 0;
    if (irq & LORA_IRQ_PAYLOAD_CRC_ERROR_MASK)
        return 0;

    // Find packet size.
    if (_implicit)
        len = lora_read_reg (LORA_REG_PAYLOAD_LENGTH);
    else
    ...
    if (len > size)
        len = size;
#if BUFFER_IO
    lora_read_reg_buffer (LORA_REG_FIFO, buf, len);
#else
    for (int i = 0; i < len; i++)
...
void lora_set_spreading_factor(int sf)
{
    ...
    if (sf == 6)
    {
        lora_write_reg (LORA_REG_DETECTION_OPTIMIZE, 0xc5);
        lora_write_reg (LORA_REG_DETECTION_THRESHOLD, 0x0c);
    }
    else
    {
        lora_write_reg (LORA_REG_DETECTION_OPTIMIZE, 0xc3);
        lora_write_reg (LORA_REG_DETECTION_THRESHOLD, 0x0a);
    }
    lora_write_reg (LORA_REG_MODEM_CONFIG_2, (lora_read_reg(LORA_REG_MODEM_CONFIG_2) & 0x0f) | ((sf << 4) & 0xf0));
    _sf = sf;
}

int lora_get_spreading_factor(void)
{
    return (lora_read_reg (LORA_REG_MODEM_CONFIG_2) >> 4);
}
...
void lora_set_bandwidth(int sbw)
{
    ...
    if (sbw < 10)
    {
        lora_write_reg (LORA_REG_MODEM_CONFIG_1, (lora_read_reg (LORA_REG_MODEM_CONFIG_1) & 0x0f) | (sbw << 4));
        _sbw = sbw;
    }
}
...
void lora_set_coding_rate(int cr)
{
    ...
    if (cr < 1)
        ...
        cr = 4;
    lora_write_reg (LORA_REG_MODEM_CONFIG_1, (lora_read_reg (LORA_REG_MODEM_CONFIG_1) & 0xf1) | (cr << 1));
    _cr = cr;
}

int lora_get_coding_rate(void)
{
    return ((lora_read_reg (LORA_REG_MODEM_CONFIG_1) & 0x0E) >> 1);
}

void lora_set_preamble_length(long length)
{
    lora_write_reg (LORA_REG_PREAMBLE_MSB, (uint8_t) (length >> 8));
    lora_write_reg (LORA_REG_PREAMBLE_LSB, (uint8_t) (length >> 0));
}

long lora_get_preamble_length(void)
{
    long preamble;
    preamble = lora_read_reg (LORA_REG_PREAMBLE_MSB) << 8;
    preamble = preamble + lora_read_reg (LORA_REG_PREAMBLE_LSB);
    return preamble;
}
...
{
    _implicit = 0;
    lora_write_reg (LORA_REG_MODEM_CONFIG_1, lora_read_reg (LORA_REG_MODEM_CONFIG_1) & 0xfe);
}

void lora_implicit_header_mode(int size)
{
    _implicit = 1;
    lora_write_reg (LORA_REG_MODEM_CONFIG_1, lora_read_reg (LORA_REG_MODEM_CONFIG_1) | 0x01);
    lora_write_reg (LORA_REG_PAYLOAD_LENGTH, size);
}

void lora_set_sync_word(int sw)
{
    lora_write_reg (LORA_REG_SYNC_WORD, sw);
}

void lora_idle(void)
{
    lora_write_reg(LORA_REG_OP_MODE, LORA_MODE_LONG_RANGE | LORA_MODE_STDBY);
}
...
{
    lora_write_reg(LORA_REG_OP_MODE, LORA_MODE_LONG_RANGE | LORA_MODE_RX_CONTINUOUS);
}

void lora_enable_crc (void)
{
    lora_write_reg(LORA_REG_MODEM_CONFIG_2, lora_read_reg (LORA_REG_MODEM_CONFIG_2) | 0x04);
}

void lora_disable_crc(void)
{
    lora_write_reg(LORA_REG_MODEM_CONFIG_2, lora_read_reg (LORA_REG_MODEM_CONFIG_2) & 0xfb);
}

int lora_received(void)
{
    if (lora_read_reg (LORA_REG_IRQ_FLAGS) & LORA_IRQ_RX_DONE_MASK)
        return 1;
    return 0;
}

int lora_get_irq(void) { return (lora_read_reg (LORA_REG_IRQ_FLAGS)); }

int lora_packet_lost(void) { return (_send_packet_lost); }

int lora_packet_rssi(void)
{
    return (lora_read_reg (LORA_REG_PKT_RSSI_VALUE) - (_frequency < 868E6 ? 164 : 157));
}

float lora_packet_snr(void) { return ((int8_t) lora_read_reg (LORA_REG_PKT_SNR_VALUE)) * 0.25; }
